...
def configure_tracing() -> None:
    ...
    langsmith.Client().create_run(...) or manually construct Langfuse spans
    themselves. With these env vars set, every node in the compiled
    StateGraph reports to one nested LangSmith trace per graph.invoke()
    call, and every @observe-decorated video-LLM backend call reports to
